...
public class ArrayMultiTreeNode<T> extends MultiTreeNode<T> {
	...
	public void clear() {
		if (!isLeaf()) {
            IntStream.range(0, subtreesSize)
                .mapToObj(i -> (TreeNode<T>) subtrees[i])
                .forEachOrdered(TreeNode::unlinkParent);
	...
	public boolean hasSubtree(TreeNode<T> subtree) {
        ...
        return IntStream.range(0, subtreesSize)
            .mapToObj(i -> (TreeNode<T>) subtrees[i])
            .anyMatch(subtree::equals);
	}
	...
	public boolean remove(TreeNode<T> node) {
        ...
        return IntStream.range(0, subtreesSize)
            .mapToObj(i -> (TreeNode<T>) subtrees[i])
            .anyMatch(subtree -> subtree.remove(node));
	}
	...
	public void traversePreOrder(TraversalAction<TreeNode<T>> action) {
		if (!action.isCompleted()) {
			...
			if (!isLeaf()) {
                IntStream.range(0, subtreesSize)
                    .mapToObj(i -> (TreeNode<T>) subtrees[i])
                    .forEachOrdered(subtree -> subtree.traversePreOrder(action));
			}
		}
	}
	...
	public void traversePostOrder(TraversalAction<TreeNode<T>> action) {
		if (!action.isCompleted()) {
			if (!isLeaf()) {
                IntStream.range(0, subtreesSize)
                    .mapToObj(i -> (TreeNode<T>) subtrees[i])
                    .forEachOrdered(subtree -> subtree.traversePostOrder(action));
			}
			action.perform(this);
		}
	}
